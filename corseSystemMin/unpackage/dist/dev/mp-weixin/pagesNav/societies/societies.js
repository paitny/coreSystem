...
const pagesNav_commons_js_myfun = require("../commons/js/myfun.js");
const submit = () => "../../components/submit/submit.js";
const _sfc_main = {
  data() {
    return {
      baseURL: "",
  ...
  methods: {
    getUserPhoto() {
      ...
      if (userInfo && userInfo.photo) {
        this.userPhoto = this.baseURL + userInfo.photo;
      } else {
        this.userPhoto = this.aiAvatar;
      }
    },
    loadMessages() {
      const storedMessages = common_vendor.index.getStorageSync("chatMessages");
      if (storedMessages) {
        this.msg = this.processMessages(JSON.parse(storedMessages));
        this.$nextTick(() => {
          this.scrollToBottom();
        });
      }
    ...
    processMessages(messages) {
      ...
      messages.forEach((message, index) => {
        ...
        if (lastTime) {
          const timeDiff = (currentTime - lastTime) / 1e3 / 60;
          if (timeDiff > 5) {
    ...
    changeTime(date) {
      return pagesNav_commons_js_myfun.myfun.dateTime(date);
    },
    backOne() {
    ...
    saveMessages() {
      common_vendor.index.setStorageSync("chatMessages", JSON.stringify(this.msg));
    },
    ...
    scrollToBottom() {
      this.$nextTick(() => {
        common_vendor.index.createSelectorQuery().select(".chat-main").boundingClientRect((res) => {
          if (res) {
  ...
  return {
    a: common_vendor.f($data.msg, (item, index, i0) => {
      return common_vendor.e({
        ...
        b: common_vendor.t($options.changeTime(item.time))
      } : {}, {
        c: item.id === "ai"
      }, item.id === "ai" ? {
        d: $data.aiAvatar,
    ...
    b: $data.inputh + "px",
    c: $data.scrollToView,
    d: common_vendor.f($data.faqList, (question, index, i0) => {
      return {
        a: common_vendor.t(question),
        b: index,
        c: common_vendor.o(($event) => $options.handleFaqClick(question), index)
      };
    }),
    e: common_vendor.o($options.submitChat),
    f: common_vendor.o($options.getSubHeight)
  };
}
