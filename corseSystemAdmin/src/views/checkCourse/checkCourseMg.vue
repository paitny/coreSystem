<template>
  <el-card>
    ...
    <div style="display: flex;align-items: center;justify-content: space-between;min-width: max-content;">
      ...
      <el-button @click="exportCheckCourseCount" type="success" plain>学期数据汇总导出</el-button>
      <div class="searchBox">
        <div>全局查询:&nbsp;</div>
      ...
      <div style="display: flex;align-items: center">
        ...
        <el-select v-model="selectedClass" placeholder="请选择班级" @change="onClassChange" style="min-width: 200px">
          <el-option v-for="classItem in filteredClasses" :key="classItem" :value="classItem">{{
              classItem
            }}
          </el-option>
        </el-select>
      ...
      >
        <template #default="scope">
          <div class="demo-images__container" >
            <el-image
                :src="`${baseURL}${staticURL}${scope.row.photo[0]}`"
                :preview-src-list="scope.row.photo.map(image => `${baseURL}${staticURL}${image}`)"
                fit="cover"
        ...
        >
          <template #default="scope">
            {{ mapWeek[scope.row.weekday] }}
          </template>
        </el-table-column>

        <el-table-column
            prop="course.rawSection"
            label="节次"
            align="center"
        >
        ...
        <el-table-column fixed="right" label="操作2" align="center" width="120">
          <template #default="scope">


            <el-button color="#7d50f4" type="success" @click="goToCheckInfo(scope.row.results)" size="default">查课数据
            </el-button>
        ...
        >
          <template #default="scope">
            <el-button
                type="danger"
                @click=" openLogout(scope.row.course.name,scope.row._id,scope.row.grade,scope.row.class,scope.row.level,scope.row.week,scope.row.weekday,scope.row.photo)"
                size="default"
            >班级删除
            </el-button>
          </template>
...
import {exportAttendanceToExcel, exportStudentAbsenceSummaryToExcel} from "../../utils/courseFaceback";
...
export default {
  ...
  data() {
    return {
      ...
      filenameTxt: "",
      regUsersData: [],
      mapWeek: ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'],
      updateFrom: {
        name: '',
        sex: '',
        grade: '',
  ...
  methods: {
    async loadStartWeek() {
      await this.$axios({
        method: "GET",
        url: "/api/adminServer/get/getClassInfoWeek",
        params: {week: this.selectedStartWeek}
      }).then(response => {
        console.log(response)
        this.userData = response.data;
        this.filenameTxt='第'+this.selectedStartWeek+'周查课数据'
      })
          .catch(error => {
    ...
    async getPointClass() {
      let {data} = await this.$axios({
        method: "GET",
        url: "/api/adminServer/checkCourse/searchClassCheckCourse",
        params: {
          grade: this.selectedGrade,
          className: this.selectedClass,
          levels: this.selectedLevel,
          termName: this.semester
        }
      })
      this.userData = data
    ...
    openLogout(name, id, grade, className, level, week, weekday, photo) {
      console.log(name)
      ElMessageBox.confirm(
          `你确定要删除${grade + className + level} ${'第'}${week}${'周'}${this.mapWeek[weekday]} 的${name}的查课记录？`,
          'Warning',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
 ...
 async   exportUser() {
      ...
      await this.$axios({
        method: "GET",
        url: "/api/adminServer/get/attendance/week",
        params: {week: this.selectedStartWeek,termName:this.semester}
      }).then(response => {
        console.log(response.data );
        
      
        this.filenameTxt='第'+this.selectedStartWeek+'周查课数据'
        exportAttendanceToExcel(response.data, this.filenameTxt)
    ...
    //导出搜索用户
    async exportCheckCourseCount() {
      let {data} = await this.$axios({
        method: "GET",
        url: "/api/adminServer/get/studentAbsenceSummary",
        params: {termName: this.semester}
      })
      console.log(data)
      exportStudentAbsenceSummaryToExcel(data, this.semester + '查课数据汇总')
    },
    //搜索
    handleSearch() {
      ...
      this.timer = setTimeout(async () => {
        ...
        let {data} = await this.$axios({
          method: "GET",
          url: "/api/adminServer/checkCourse/searchCheckCourse",
          params: {query: txt}
        })
        this.userData = []
    ...
    async getUserData() {
      ...
      let {data} = await this.$axios({
        method: "GET",
        url: "/api/adminServer/get/getClassInfo",
        params: this.query
      })
      this.filenameTxt="查课数据"
      if (data.code) {
        return this.$message.error(data.msg)
      }
    ...
    //用户注销
    async deleteUser(id, photo) {
      let {data} = await this.$axios({
        method: "DELETE",
        url: "/api/adminServer/checkCourse/deleteCourseFeedback",
        data: {id, photo}
      })
      if (data.code) {
        return this.$message.error(data.msg)
    ...
    async goToCheckInfo(result) {
      let results = JSON.stringify(result)
      this.$router.push({name: 'checkCourse', query: {id: results}});
    }
  },
  mounted() {
    this.getGRLInfo()
